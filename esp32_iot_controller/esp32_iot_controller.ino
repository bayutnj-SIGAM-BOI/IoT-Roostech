 ...
 *    - Servo           : feeder otomatis
 *    - Relay x2        : lampu & pompa
 *    - MQTT (HiveMQ)   : telemetri & command
 *
 *  Pin LCD I2C:
 *    SDA → GPIO 14
 *    SCL → GPIO 15
...
void updateLCD() {
    ...
    if (lcdPage == 0) {
        ...
        lcd.print(rssiStr);   // FIX: tampilkan rssiStr ke LCD
    }
    lcdPage = 1 - lcdPage;
}

// ════════════════════════════════════════════════════════════
//  lcdShowLine — tulis 1 baris penuh ke LCD (auto-pad spasi)
// ════════════════════════════════════════════════════════════
void lcdShowLine(int row, const String& text) {
    lcd.setCursor(0, row);
    String padded = text;
    while (padded.length() < LCD_COLS) padded += ' ';   // FIX: gunakan LCD_COLS
